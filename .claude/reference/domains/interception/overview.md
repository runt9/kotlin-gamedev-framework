...
- [fact] As of this reading, nothing calls the non-filtering two-argument overload — not in kgdfw, and not in RogueFlip (its 29 dispatch sites all use the single-arg form or their own wrapper). It is dead API rather than a live hazard, but it will bypass matching if someone reaches for it.
