...
2. **Decide the ticket-tag prefix.** `_validate.py` accepts any `#ABC-1234` shape by pattern. **kgdfw has no issue tracker** — work here is driven from the consuming project's issues, so reference those in prose, naming the project, rather than as inline tags.
...
that only makes sense if you already know RogueFlip exists is in the wrong repository.
...
Cross-repo references are written as an explicit relative path in prose — for example "RogueFlip's
`.claude/reference/domains/interceptors/overview.md`" — and **never as a `[[wikilink]]`**. A wikilink that
can never resolve inside this corpus reads as a note somebody still owes, which is the opposite of what it
means.
...
- [[Player Model]] — how RogueFlip is actually *played*, built from recorded play with narration. Read before reasoning about flip behaviour, board UX, item balance or difficulty tuning; the code says what the game permits, not what a player does.
- [[Item Catalog]] — mechanical cache of every Sticker, Trinket, Card Upgrade and bonus. What items *do*, not how they are valued.
- [[Metrics Schema]] — the metrics database's tables, views and the traps in querying them.

Two indexes sit one level down and are worth knowing about: [[Item Notes Index]] enumerates the per-item notes in `entities/items/`, and [[Recording Ingest]] enumerates the recordings in `entities/recordings/` as well as documenting how a video becomes notes.
